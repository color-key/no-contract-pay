...
public class OrderServiceimpl implements OrderService {
    ...
    @Override
    public Page<Order> fenyeorder(String merchid,Integer qrtype, Integer state, int pageNum, int pageSize) {
    //当前页
    	int currentPage=(pageNum-1)*pageSize;
        System.out.println("起始页"+currentPage);
        List<Order> list= orderMapper.fenyeorder(merchid,qrtype, state, currentPage,pageSize);
        System.out.println(list+"**************");
       //总条数
//        int  allSizes=getCount(merchid);
        int allSizes = orderMapper.getOrderCount(merchid, qrtype, state);
    ...
    @Override
    public Page<Order> queryorder(String uid,String ordernumber, String cus_merchid, Double djmoney, int qrtype, int pageNum, int pageSize,String begintime,String endtime,Integer state) {
        //当前页
        int    currentPage=(pageNum-1)*pageSize;
        System.out.println("起始页"+currentPage);
        List<Order>   list=orderMapper.findorder(uid,ordernumber,cus_merchid,djmoney,qrtype,currentPage,pageSize,begintime,endtime,state);
